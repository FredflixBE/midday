...
export const getTeamById = async (db: Database, id: string) => {
  const [result] = await db
    .select({
      ...
      // The business's own identity (FF-1641), which invoices and quotes
      // both print. Held on the team so there is one of it.
    ...
    if (parentId) {
      for (const child of parent.children) {
        ...
        childCategoriesToInsert.push({
          ...
          canHaveSupplierInvoice: child.canHaveSupplierInvoice,
          taxRate: taxRate > 0 ? taxRate : null,
          taxType: taxRate > 0 ? taxType : null,
          taxReportingCode: undefined,
          description: undefined,
