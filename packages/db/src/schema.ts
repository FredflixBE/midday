...
/**
 * One customer's own hourly rate for a product (FF-1620): a quote line picks
 * a product, and its price is the hourly rate unless the customer has one
 * here, or the quote its own. No row: the product's price applies.
 */
export const customerProductRates = pgTable(
  ...
  (table) => [
    primaryKey({
      ...
      name: "customer_product_rates_pkey",
    }),
    index("customer_product_rates_team_id_idx").on(table.teamId),
    index("customer_product_rates_product_id_idx").on(table.productId),
    foreignKey({
      columns: [table.customerId],
      foreignColumns: [customers.id],
      name: "customer_product_rates_customer_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.productId],
      foreignColumns: [invoiceProducts.id],
      name: "customer_product_rates_product_id_fkey",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.teamId],
      foreignColumns: [teams.id],
      name: "customer_product_rates_team_id_fkey",
    }).onDelete("cascade"),
    pgPolicy("Customer product rates can be handled by members of the team", {
      as: "permissive",
      for: "all",
      to: ["public"],
      using: sql`(team_id IN ( SELECT private.get_teams_for_authenticated_user() AS get_teams_for_authenticated_user))`,
