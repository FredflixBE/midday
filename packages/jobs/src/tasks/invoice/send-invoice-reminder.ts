...
import {
  type SendInvoiceReminderPayload,
...
export class SendInvoiceReminderProcessor extends BaseProcessor<SendInvoiceReminderPayload> {
  async process(job: JobContext<SendInvoiceReminderPayload>): Promise<void> {
...
const processor = new SendInvoiceReminderProcessor();
...
export const sendInvoiceReminder = schemaTask({
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "send-invoice-reminder", payload, ctx),
});
