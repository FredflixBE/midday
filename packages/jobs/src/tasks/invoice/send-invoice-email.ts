...
export class SendInvoiceEmailProcessor extends BaseProcessor<SendInvoiceEmailPayload> {
  async process(job: JobContext<SendInvoiceEmailPayload>): Promise<void> {
...
const processor = new SendInvoiceEmailProcessor();
...
export const sendInvoiceEmail = schemaTask({
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "send-invoice-email", payload, ctx),
});
