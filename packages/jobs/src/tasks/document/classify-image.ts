...
import { resizeImage } from "@jobs/utils/image-processing";
    ...
    if (classificationResult?.tags && classificationResult.tags.length > 0) {
      ...
      await embedDocumentTags.trigger(
        { documentId: data.id, tags: classificationResult.tags, teamId },
        {
          ...
          // the same image does not queue the same work twice.
...
const processor = new ClassifyImageProcessor();

export const classifyImage = schemaTask({
  id: "classify-image",
  schema: classifyImageSchema,
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "classify-image", payload, ctx),
