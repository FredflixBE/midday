...
import { handleJob, toTaskError } from "@jobs/processors/run";
  ...
  async process(job: JobContext<ProcessDocumentPayload>): Promise<void> {
    ...
    try {
      ...
      if (processedMimetype.startsWith("image/")) {
        ...
        // Wait for classification so its errors surface here and this document
        // reaches a terminal status. The child's own maxDuration bounds the
        // wait; there is no separate timeout to keep in step with it.
        const imageResult = await classifyImage.triggerAndWait({
          fileName,
          teamId,
        });
      ...
      // Wait for classification so its errors surface here and this document
      // reaches a terminal status. The child's own maxDuration bounds the wait.
      const classificationJobResult = await classifyDocument.triggerAndWait({
        content: sample,
        fileName,
        teamId,
      });

      if (!classificationJobResult.ok) {
        throw new Error(
  ...
  run: async (payload, { ctx }) => {
    try {
      ...
      if (error instanceof UnsupportedFileTypeError) {
        await markDocumentUnsupported({
          pathTokens: payload.filePath,
