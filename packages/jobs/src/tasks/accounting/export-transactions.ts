...
import type { AccountingExportPayload } from "@jobs/schemas/accounting";
import {
  accountingExportSchema,
  providerEntityTypeSchema,
} from "@jobs/schemas/accounting";
import {
  AccountingProcessorBase,
  type AccountingProviderId,
} from "@jobs/tasks/accounting/base";
...
import { syncAccountingAttachments } from "./sync-attachments";
...
export class ExportTransactionsProcessor extends AccountingProcessorBase<AccountingExportPayload> {
  async process(job: JobContext<AccountingExportPayload>): Promise<{
    teamId: string;
    providerId: string;
          ...
          for (const txResult of result.results) {
            ...
            if (txResult.success && txResult.providerTransactionId) {
              ...
              if (attachments.length > 0) {
                ...
                await syncAccountingAttachments.trigger(
                  {
                    teamId,
                    providerId,
                    transactionId: txResult.transactionId,
        ...
        await syncAccountingAttachments.trigger(
          {
            ...
            // The column is a plain string; the task's schema narrows it to
            // the four entity types. Parsing rather than casting means an
            // unrecognised value arrives as undefined instead of failing
            // validation on the far side.
            providerEntityType: providerEntityTypeSchema.safeParse(
              item.syncRecord.providerEntityType,
            ).data,
          },
          { delay: new Date(Date.now() + delay) },
        );
...
const processor = new ExportTransactionsProcessor();

export const exportToAccounting = schemaTask({
  id: "export-to-accounting",
  schema: accountingExportSchema,
  // The export bar watches this run's progress.
  maxDuration: 900,
  queue: { concurrencyLimit: 10 },
  retry: {
    maxAttempts: 4,
    minTimeoutInMs: 5 * 60 * 1000,
    maxTimeoutInMs: 20 * 60 * 1000,
    factor: 2,
  },
  run: (payload, { ctx }) =>
    runProcessor(processor, "export-to-accounting", payload, ctx),
});
