...
import {
  type AccountingAttachmentSyncPayload,
  accountingAttachmentSyncSchema,
...
export class SyncAttachmentsProcessor extends AccountingProcessorBase<AccountingAttachmentSyncPayload> {
  async process(job: JobContext<AccountingAttachmentSyncPayload>): Promise<{
    teamId: string;
    providerId: string;
    ...
    const newAttachmentIds = attachmentIds.filter(
      (id: string) => !alreadySyncedIds.has(id),
    );
...
const processor = new SyncAttachmentsProcessor();

export const syncAccountingAttachments = schemaTask({
  id: "sync-accounting-attachments",
  schema: accountingAttachmentSyncSchema,
  // Uploads run under a provider rate limit, so a single run can be slow.
  maxDuration: 900,
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "sync-accounting-attachments", payload, ctx),
});
