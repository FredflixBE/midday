...
export class ExportTransactionsProcessor extends BaseProcessor<ExportTransactionsPayload> {
  async process(job: JobContext<ExportTransactionsPayload>): Promise<{
    filePath: string;
    fullPath: string;
    fileName: string;
    totalItems: number;
...
export const exportTransactions = schemaTask({
  ...
  // The dashboard watches this run's progress, and a large export downloads
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "export-transactions", payload, ctx),
});
