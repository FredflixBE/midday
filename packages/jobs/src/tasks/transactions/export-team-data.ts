...
import {
  type ExportTeamDataPayload,
  exportTeamDataSchema,
} from "@jobs/schemas/transactions";
import { processTransactionsForExport } from "@jobs/utils/process-export";
...
export class ExportTeamDataProcessor extends BaseProcessor<ExportTeamDataPayload> {
  async process(job: JobContext<ExportTeamDataPayload>): Promise<{
    filePath: string;
    fullPath: string;
    fileName: string;
    totalItems: number;
    ...
    for (let i = 0; i < transactionIds.length; i += TX_BATCH) {
      ...
      const result = await processTransactionsForExport({
        ids: batch,
        teamId,
...
const processor = new ExportTeamDataProcessor();

export const exportTeamData = schemaTask({
  id: "export-team-data",
  schema: exportTeamDataSchema,
  // A full team export downloads every invoice PDF, vault document and inbox
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "export-team-data", payload, ctx),
});
