...
export class SlackUploadProcessor extends BaseProcessor<SlackUploadPayload> {
  async process(job: JobContext<SlackUploadPayload>): Promise<void> {
...
const processor = new SlackUploadProcessor();

export const slackUpload = schemaTask({
  id: "slack-upload",
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "slack-upload", payload, ctx),
});
