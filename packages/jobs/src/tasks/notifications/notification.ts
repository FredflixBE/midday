...
} from "@jobs/schemas/notifications";
...
export class NotificationProcessor extends BaseProcessor<NotificationPayload> {
  async process(job: JobContext<NotificationPayload>): Promise<void> {
...
export const notification = schemaTask({
  ...
  machine: "micro",
  maxDuration: 60,
  // The notifications queue rate-limited to 5/second to spare the email
  // provider; the concurrency limit is the closest equivalent here.
  queue: { concurrencyLimit: 5 },
  retry: { maxAttempts: 3, minTimeoutInMs: 1000, factor: 2 },
  run: (payload, { ctx }) =>
    runProcessor(processor, "notification", payload, ctx),
});
