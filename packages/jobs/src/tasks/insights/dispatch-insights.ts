...
import { z } from "zod";
import { generateTeamInsights } from "./generate-team-insights";
...
export class DispatchInsightsProcessor extends BaseProcessor<DispatchInsightsPayload> {
  async process(
    job: JobContext<DispatchInsightsPayload>,
    ...
    while (true) {
      ...
      for (const team of eligibleTeams) {
        ...
        try {
          await generateTeamInsights.trigger(
            {
              teamId: team.id,
              periodType,
              periodYear: period.periodYear,
            ...
            {
              // Was a BullMQ job id: one insight per team and period, however
              // often the dispatcher runs across the window.
              idempotencyKey: `insights-${team.id}-${periodType}-${period.periodYear}-${period.periodNumber}`,
              idempotencyKeyTTL: "7d",
              delay: new Date(Date.now() + jobDelay),
            },
          );
...
const processor = new DispatchInsightsProcessor();

// Insights are on hold: this stays triggerable but FF-1387 registers no
// schedule for it, so nothing fans out on its own.
export const dispatchInsights = schemaTask({
  ...
  run: (payload, { ctx }) =>
    runProcessor(processor, "dispatch-insights", payload, ctx),
});
