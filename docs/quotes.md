...
- **Quote editor** as a full page (a quote is too big for the invoice sheet), laid out so the page reads as the quote rather than as a form (FF-1629): a main column that is two tabs, a settings rail beside it, and a strip of identity and actions across the top.
  - the **main column is two tabs** (FF-1639), because writing a quote and pricing it are two jobs and the second is a form:
    - **Document** — the title and the text blocks, drawn as the PDF draws them (FF-1633): the measure of the PDF's text column, its heading scale, its lists, marks and pictures, so the page is the preview and downloading a PDF is no longer the only way to see the work. One thing still differs: a blank line typed between two paragraphs shows on screen and is dropped by the PDF (**FF-1637**). Editing stays in place and reaches for nothing (FF-1638): typing `/` offers what a block can hold — text, the three heading levels, the two kinds of list, a picture — a selection raises the bubble menu that marks it up, the markdown shortcuts still work, and the heading is typed where the heading is. The only controls a block has are its grip, expand and remove, and those float above it and appear only while it is under the pointer or holds focus, so a quote at rest reads as a quote. A heading is content and always shows; an **empty** heading is a field, and waits out of sight with the rest of the chrome, keeping its line so nothing moves when it appears (FF-1644). The `/` menu offers only what every renderer draws, so nothing can be written that the client would never see. The blocks are reorderable, with the pricing block among them. That block **shows what prints there** (FF-1640) — the comparison table and each scenario as the PDF lays them out — and collapses to a line with its label. Its place among the text is the only thing that decides where the tables go, so it drags either way, and it collapses while it is dragged: a block the height of a page is not one you can drop where you meant to. Nothing is edited or reached from there: the Pricing tab is where the numbers are changed, and the tab strip is the way to it.
    - **Pricing** — the scenarios as tabs, each tab a line table (drag to reorder; product picker; hours or min–max; optional toggle; one-off toggle for recurring) with its recurrence and payment settings, and the internal comparison panel at the foot, which is for the person writing the quote and not for the client but belongs with the priced content.

    The page is capped at the document's measure plus the rail — 800 + 48 + 380 — and at the same width on both tabs, so nothing moves when you switch: a page that changed width would take the tab you had just clicked out from under the pointer. The priced tables would like more room than that and do not get it; instead they are sized for the column they have, and a scenario that asks for one quantity rather than two, or has no one-off column, gives that width back to the item.

    **The tabs are the interim, not the destination.** The end state is still the one in §7's *Later*: the pricing block rendering the comparison and scenario tables as they print, editable in place. When that happens the Pricing tab's content moves back inline and the tabs fold away.
...
A new react-pdf template in `packages/quote`, downloaded from the dashboard. What the pricing *says* — the comparison table and every scenario's rows, subtotals and totals — is worked out by `pricingView` in `pdf/document.ts`, exported as `@midday/quote/view` so the editor draws the same thing on screen without reaching for react-pdf (FF-1640): one set of words and numbers, laid out twice. It is the reference for what a quote says: the editor's document pane borrows its proportions (9pt body, headings at 14, 12 and 10pt, lists indented by 12pt) and repeats none of its logic. It's served the way the invoice download is (`apps/api/src/rest/routers/files/download.ts`). In order:
...
**One reading rhythm, four surfaces** (FF-1651). The same text is drawn four ways — the editor on screen, the PDF the client is sent, the web view and the MCP preview — and only one of them can read a stylesheet. react-pdf takes numbers, so `packages/invoice/src/templates/typeset.ts` is where the scale lives: a heading's size as a multiple of the body size, the leading, and the flow — the space above and below a heading. Each surface says only how big its own body text is (9pt in print, 11px on the web view and in the preview, 14px in the editor) and every other size follows. That is what keeps the screen reading as the print reads: one set of proportions, not four sets of numbers kept in step by hand.

The steps are 1.2 and 1.45 from the body for a subheading and a heading, 1.75 for a block's own title — which *is* the document's h1, so the slash menu offers nothing above it — and 2.1 for the quote's own name in the strip. Every paragraph takes room under it as well, but only where it is asked for: the same renderer draws an invoice's address blocks, and an address is a list of lines rather than a run of paragraphs. The PDF's own header and pricing furniture read from the same scale (FF-1653): the title leads at the top of it and the quote number identifies the document in small text above, rather than the other way round. The scale this replaced stepped 1.11 from the body at the small end, and a heading 11% larger than the text under it is not a heading.

**A table is drawn wherever the text is drawn** (FF-1642). A proposal lays out comparison tables of its own — feature matrices, before-and-after — which are not the `Vergelijking` the pricing block generates from the scenarios; that one is still worked out by `pricingView` and is untouched. A written table is four Tiptap nodes (`table`, `tableRow`, `tableHeader`, `tableCell`), and they are part of the schema wherever `registerExtensions` builds one, on the same terms pictures are: Tiptap throws away nodes its schema does not know, so an editor built without them would strip a table out of text it was only meant to show. Only the quote's document pane offers to *write* one, though — `/table` and a bar of row and column controls that floats while the caret is inside a table — so an invoice note stays the few lines of text it is.

Every column takes an equal share of the text column, in the PDF, on the web view, in the MCP preview and in the editor alike. Nothing is sized by what a cell holds: react-pdf cannot measure text before it lays it out, so a content-sized column would be a column the printed quote disagreed with, and for the same reason columns cannot be dragged. A table is never wider than the text column. A row is never divided over two pages; a table longer than a page breaks between rows, and its header row does not repeat on the next one. The OG card draws no grid — there is no room for one at 1200×630 — but it keeps the words, a line per cell.

**A diagram is drawn once, when it is written** (FF-1643). A quote's text may hold a mermaid diagram, and the node keeps two things: the mermaid source, which is what makes it editable again, and the path of the PNG that source was drawn into. Only the second is ever drawn. Mermaid draws SVG in a browser, and react-pdf draws neither the whole of SVG nor the `foreignObject` mermaid puts its labels in — so letting each renderer draw it would mean a second renderer for the PDF or a headless browser at send time. Turning it into a picture costs the renderers nothing, because they already draw pictures, and it goes into the `vault` under `<team>/quotes/` through the same upload as any other picture in the text (FF-1625) — so FF-1626 lets go of it on the same terms. Mermaid is loaded only once a diagram is actually open, so a quote holding ten of them loads none of it.

A stored picture is never redrawn on its own. Mermaid's output shifts between versions, and a quote that redrew itself because a dependency moved would be a document nobody edited that no longer says what its author approved. It is drawn again when someone opens the diagram and saves it, and only then. A diagram written but not yet drawn, or one whose picture cannot be read, is left out of the PDF rather than failing it.
