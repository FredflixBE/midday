...
sequenceDiagram
    ...
    Cron->>Scheduler: Daily, after the warnings
...
sequenceDiagram
    ...
    Note over DB: WHERE next_scheduled_at BETWEEN now AND now + 24h<br/>AND not already warned for this cycle
...
**The look-ahead has to equal the gap between runs, and does.** The window is
`(now, now + 24h]`, and the generation takes everything with
`next_scheduled_at <= now`. When the two match, consecutive windows tile the
timeline — no gap, no overlap — so a series falls in exactly one of them, is
warned there, and is generated by the very next run. The warning therefore
always arrives one day before the invoice goes out, which is what the email's
"scheduled for tomorrow" claims. `RECURRING_INVOICE_CRON` and
`WARNING_LOOK_AHEAD_HOURS` live together in
[`packages/jobs/src/utils/recurring-invoice-day.ts`](../packages/jobs/src/utils/recurring-invoice-day.ts),
with a test that they still agree.

Two kinds of series get no warning, where the hourly job gave them a few hours'
notice: one that falls due within a day of being created or edited, and one
that recurs daily. Both are the cadence rather than a defect in the window — a
run cannot warn a day ahead about something less than a day away.
...
`WARNING_LOOK_AHEAD_HOURS + 1` before the due date belongs to this cycle rather
than the last, which is the boundary both the query and the processor's own
filter derive from that constant.
