...
function CopyButton({ path }: { path: string }) {
  ...
  return (
    <button type="button" onClick={handleCopy}>
      {isCopied ? (
        <Icons.Check className="size-4 text-foreground dark:text-muted-foreground hover:!text-foreground cursor-pointer" />
      ) : (
        <Icons.Copy className="size-4 text-foreground dark:text-muted-foreground hover:!text-foreground cursor-pointer" />
      )}
    </button>
  );
}
  ...
  return (
    ...
    >
      {isDownloading || isLoading ? (
        ...
        <Icons.ArrowCoolDown className="size-4 text-foreground dark:text-muted-foreground hover:!text-foreground cursor-pointer" />
      )}
    </button>
  );
}
...
const SearchResultItemDisplay = ({
  ...
  if (!item.data) {
    ...
    icon = (
      <Icons.Shortcut className="size-4 text-foreground dark:text-muted-foreground" />
    );
    ...
    switch (item.type) {
        ...
        resultDisplay = (
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-2 invisible group-hover/item:visible group-focus/item:visible group-aria-selected/item:visible">
              ...
              />
              <Icons.ArrowOutward className="size-4 text-foreground dark:text-muted-foreground hover:!text-foreground cursor-pointer" />
            </div>
          </div>
        );
        break;
      ...
      case "customer": {
        ...
        icon = (
          <Icons.Customers className="size-4 text-foreground dark:text-muted-foreground" />
        );
        resultDisplay = (
      ...
      case "invoice": {
        ...
        icon = (
          <Icons.Invoice className="size-4 text-foreground dark:text-muted-foreground" />
        );
        resultDisplay = (
          <div className="flex items-center w-full">
            <div className="grow truncate flex gap-2 items-center">
      ...
      case "transaction": {
        ...
        icon = (
          <Icons.Transactions className="size-4 text-foreground dark:text-muted-foreground" />
        );
        resultDisplay = (
          <div className="flex items-center justify-between w-full">
            <div className="grow truncate flex gap-2 items-center">
...
export function Search() {
  ...
  return (
    <Command
      shouldFilter={false}
      className="search-container overflow-hidden p-0 relative w-full backdrop-filter backdrop-blur-lg bg-background/[99] h-auto border border-border"
    >
      <div className="border-b border-border relative">
        ...
        {isFetching && (
          <div className="absolute bottom-0 h-[2px] w-full overflow-hidden">
            <div className="absolute top-[1px] h-full w-40 animate-slide-effect bg-gradient-to-r from-muted via-foreground via-80% to-muted" />
          </div>
        )}
      </div>
