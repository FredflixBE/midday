 ...
 * from one menu. Every line decomposes into the payments listed below it.
 */
...
function CommitmentRow({ commitment }: { commitment: Commitment }) {
  ...
  const update = useMutation(
    trpc.commitments.update.mutationOptions({
      onSuccess: () =>
    ...
    >
      <div className="min-w-0 truncate">
        ...
        <span className="text-[#878787]">
          ...
          {first ? ` since ${formatDate(first.date, user?.dateFormat)}` : null}
          {commitment.nextDate
